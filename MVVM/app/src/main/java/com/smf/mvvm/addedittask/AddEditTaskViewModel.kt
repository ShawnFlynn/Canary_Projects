...
import com.smf.mvvm.SingleLiveEvent
...
class AddEditTaskViewModel(
    ...
    fun saveTask() {
        val task = Task(title.get().toString(), description.get().toString())
        if (task.isEmpty) {
        ...
        if (isNewTask) {
            ...
            taskId?.let {
                updateTask(Task(title.get().toString(), description.get().toString(), it)
